...
def test(args_obj, config_file):
    ...
    cp_files(
        test_scripts_path,
        [train_file_path, model_file_path, dataset_file_path, config_file],
    )
    ...
    if args.ckpt is not None:
        start_epoch, min_val_loss, min_train_loss = load_state(
            args.ckpt,
            model,
            optimizer=None,
            map_location=device,
            ignore_keys=model.ignore_keys,
        )
    ...
    if args.test_on_train:
        Logger.log(
            "WARNING: running evaluation on TRAINING data as requested...should only be used for debugging!"
        )
    ...
    if args.eval_full_test:
        ...
        for i, data in enumerate(test_loader):
            ...
            loss, stats_dict = model_class.step(
                model,
                loss_func,
                data,
                test_dataset,
...
def viz_eval_samp(
    ...
    for b in range(B):
        ...
        # pred
        body_pred = bm_world(
            pose_body=pred_world_pose_body[b],
            pose_hand=None,
        ...
        if cur_offscreen:
            create_video(
                out_path_list[b] + "/frame_%08d." + "%s" % ("png"),
                out_path_list[b] + ".mp4",
                30,
            )
