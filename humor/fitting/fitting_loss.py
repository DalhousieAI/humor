...
class FittingLoss(nn.Module):
    ...
    def __init__(
        ...
        self.can_reproj = (
            self.smpl2op_map is not None
            and self.cam_f is not None
            and self.cam_cent is not None
        )
        if self.can_reproj:
        ...
        if (
            init_motion_prior is not None
            and sum_loss_weights["init_motion_prior"] > 0.0
        ):
        ...
        if self.robust_loss not in robust_choices:
            Logger.log(
                ...
                % (self.robust_loss, str(robust_choices))
            )
            exit()
    ...
    def root_fit(self, observed_data, pred_data):
        ...
        if (
            ...
            cur_loss = self.joints3d_loss(
                observed_data["joints3d"], pred_data["joints3d"]
            )
        ...
        use_reproj_loss = (
            ...
            and "joints3d" in pred_data
            and "joints3d_extra" in pred_data
            and self.loss_weights["joints2d"] > 0.0
        )
        if use_reproj_loss:
            if not self.can_reproj:
                Logger.log(
                    "Must provide camera intrinsics and SMPL to OpenPose joint map to use re-projection loss!"
                )
                exit()
            cur_loss = self.joints2d_loss(
                observed_data["joints2d"],
                pred_data["joints3d"],
        ...
        if (
            "seq_interval" in observed_data
            and "verts3d" in pred_data
            and self.loss_weights["rgb_overlap_consist"] > 0.0
        ):
            ...
            overlap_lens = (
                observed_data["seq_interval"][:-1, 1]
                - observed_data["seq_interval"][1:, 0]
            )
            cur_pos_loss = 0.0
            cur_vel_loss = 0.0
            ...
            if "prev_batch_overlap_res" in observed_data:
                ...
                cur_ov = (
                    (
                        observed_data["prev_batch_overlap_res"]["seq_interval"][1]
                        - observed_data["seq_interval"][0, 0]
                    )
                    .cpu()
                    .item()
                )
                ...
                # positions
                prev_pos = observed_data["prev_batch_overlap_res"]["verts3d"][-cur_ov:][
                    :ov_len
                ]
    ...
    def smpl_fit(self, observed_data, pred_data, nsteps):
        ...
        if (
            "seq_interval" in observed_data
            and "betas" in pred_data
            and self.loss_weights["rgb_overlap_consist"] > 0.0
        ):
            cur_loss = self.joints3d_loss(
                pred_data["betas"][:-1, :], pred_data["betas"][1:, :]
            )
    ...
    def motion_fit(
        ...
        if "latent_motion" in pred_data and self.loss_weights["motion_prior"] > 0.0:
            cur_loss = self.motion_prior_loss(
                pred_data["latent_motion"], cond_prior=cond_prior
            )
        ...
        if have_init_prior_info and self.loss_weights["init_motion_prior"] > 0.0:
            cur_loss = self.init_motion_prior_loss(
                pred_data["joints3d"][:, 0:1],  # only need first step
        ...
        if (
            ...
            cur_loss = self.joint_consistency_loss(
                pred_data["joints3d"], pred_data["joints3d_rollout"]
            )
        ...
        if (
            ...
            cur_loss = self.contact_vel_loss(
                pred_data["contacts_conf"], pred_data["joints3d"]
            )
        ...
        if (
            ...
            cur_loss = self.contact_height_loss(
                pred_data["contacts_conf"], pred_data["joints3d"]
            )
        ...
        if (
            ...
            and "floor_plane" in cam_pred_data
            and "floor_plane" in observed_data
        ):
            cur_loss = self.floor_reg_loss(
                cam_pred_data["floor_plane"], observed_data["floor_plane"]
            )
    ...
    def joints2d_loss(
        ...
        if cam_R is None:
            cam_R = (
                torch.eye(3).reshape((1, 3, 3)).expand((B * T, 3, 3)).to(joints3d_pred)
            )
        else:
        ...
        joints2d_pred = perspective_projection(
            joints3d_op, cam_R, cam_t, cam_f, cam_cent
        )
        ...
        if self.ignore_op_joints is not None:
            joints2d_obs_conf[
                :, :, self.ignore_op_joints
            ] = 0.0  # set confidence to 0 so not weighted
    ...
    def contact_height_loss(self, contacts_conf, joints3d):
        ...
        floor_diff = F.relu(
            torch.torch.abs(joints3d[:, :, :, 2]) - CONTACT_HEIGHT_THRESH
        )
