...
def prepare_data(data, use_overlap_loss, res_out_path):
    ...
    obs_img_paths = (
        None if "img_paths" not in observed_data else observed_data["img_paths"]
    )
    obs_mask_paths = (
        None if "mask_paths" not in observed_data else observed_data["mask_paths"]
    )
...
def process_batch(
    ...
    if _data.cur_res_out_paths is not None:
        _data.detach()
        save_optim_result(
            _data.cur_res_out_paths,
            optim_result,
            per_stage_results,
            _data.gt_data.as_dict(),
            _data.observed_data.as_dict(),
            args.data_type,
    ...
    if use_overlap_loss:
        ...
        prev_batch_overlap_res_dict["verts3d"] = (
            per_stage_results["stage3"]["verts3d"][-1].clone().detach()
        )
        prev_batch_overlap_res_dict["betas"] = (
            optim_result["betas"][-1].clone().detach()
        )
        prev_batch_overlap_res_dict["floor_plane"] = (
            optim_result["floor_plane"][-1].clone().detach()
        )
        prev_batch_overlap_res_dict["seq_interval"] = (
            _data.observed_data["seq_interval"][-1].clone().detach()
        )
...
def main(args, config_file):
    ...
    if B == 3:
        Logger.log(
            "Cannot use batch size 3, setting to 2!"
        )  # NOTE: bug with pytorch 3x3 matmul weirdness
        B = 2
    dataset = None
    ...
    if args.data_type == "AMASS":
        ...
        if not os.path.exists(video_preprocess_path):
            ...
            img_folder, _, img_shape = video_to_images(
                args.data_path, fps=30, img_folder=img_folder, return_info=True
            )
            print(img_folder)
            print(img_shape)
            # OpenPose on images
            ...
            run_openpose(
                ...
                video_out=os.path.join(
                    video_preprocess_path, "op_keypoints_overlay.mp4"
                ),
            )

            # if desired segmentation mask on images
            if args.mask_joints2d:
                run_deeplab_v3(img_folder, img_shape[:2], mask_out_path, batch_size=4)
        else:
            ...
            if args.mask_joints2d and not os.path.exists(mask_out_path):
                print(
                    "Could not find detected masks from previous pre-processing! Please delete rgb_preprocess directory and re-run!"
                )
                exit()
        ...
        if args.rgb_intrinsics is None:
            cam_mat = np.array(
                [
                    [DEFAULT_FOCAL_LEN[0], 0.0, img_shape[1] / 2.0],
                    [0.0, DEFAULT_FOCAL_LEN[1], img_shape[0] / 2.0],
                    [0.0, 0.0, 1.0],
                ]
            )
        else:
        ...
        raise NotImplementedError(
            "Fitting for arbitrary RGB-D videos is not yet implemented"
        )
    ...
    motion_prior = HumorModel(
        in_rot_rep=args.humor_in_rot_rep,
    ...
    if motion_prior is not None and max_loss_weights["init_motion_prior"] > 0.0:
        Logger.log(
            "Loading initial motion state prior from %s..." % (args.init_motion_prior)
        )
